...
<script setup>
// Fonction pour créer l'input file et gérer la sélection d'image
function selectImage(afterSelection) {
    ...
    inputFile.click(); // Simule le clic pour ouvrir le sélecteur de fichiers
}

// Fonction pour lire l'image (à implémenter selon ton besoin)
...
// Fonction pour réduire l'image (à implémenter selon ton besoin)
function reduceImage(image, callback) {
    // Implémente ici la logique pour réduire l'image (si nécessaire)
    // Et ensuite appelle le callback avec l'image réduite
    callback(image);
}

// Ajout de l'événement sur le bouton "Upload new image"
document.getElementById('uploadButton').addEventListener('click', function() {
    selectImage(function(inputFile) {
        readImage(inputFile, function(image) {
            reduceImage(image, function(imageResult) {
                document.querySelector('.img-account-profile').src = imageResult.src;
                // Ici, tu peux aussi ajouter le code pour uploader l'image vers le serveur
            });
        });
    });
});
